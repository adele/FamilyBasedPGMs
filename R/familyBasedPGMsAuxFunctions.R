    ...
    if (sigma2XS_g == 0 || sigma2YS_g == 0) {
      ...
      if (sigma2XS_e == 0 && sigma2YS_e == 0) {
        ...
        results_g <- getUnconfoundedEstimates(BgXS, BgYS, AgXS, AgYS, maxFC, minK, orthogonal,
                                              ranefXS, ranefYS, preComputedResults, useGPU, debug=debug)
  ...
  if (!results_e_done) {
    if (sigma2XS_e == 0 || sigma2YS_e == 0) {
      ...
      if (sigma2XS_g == 0 && sigma2YS_g == 0) {
        ...
        results_e <- list()
        results_e[[1]] <- list(k=N, p.value=rho_e$p.value, estimate=rho_e$estimate, sdx=sd(resXS), sdy=sd(resYS), FCxs=0, FCys=0)
      } else {
        preComputedResults <- NULL
        if (results_e_filepath != "" && file.exists(results_e_filepath)) {
        ...
        results_e <- getUnconfoundedEstimates(BeXS, BeYS, AeXS, AeYS, maxFC, minK,
                                              orthogonal, resXS, resYS, preComputedResults, useGPU, debug=debug)
...
getUnconfoundedEstimates <- function(Bxs, Bys, Axs, Ays, maxFC, minK, orthogonal, rx, ry,
                                     preComputedResults=NULL, useGPU=FALSE, gpuCores=5, debug=TRUE) {
  n = dim(Bxs)[1]
  k = minK
  FCxs = FCys = 0
  ...
  if (useGPU && requireNamespace("gpuR", quietly = TRUE)) {
    ...
    funGPU <- function(k) {
      ...
      tryCatch({
        ...
        return(list(k=k, p.value=ctest$p.value, estimate=ctest$estimate, sdx=sd(rot_rx), sdy=sd(rot_ry), FCxs=FCxs, FCys=FCys))
      },
      error = function(e) {
        cat(paste0("Error: ", e))
        return(list(e))
    ...
    cl <- gpuR::makeCluster(gpuCores, outfile="") # log can only be seen using outfile=""
    gpuR::clusterExport(cl, c("funGPU", "mcrotate_orthogonal", "mymcrotate", "myginv", "computeFC", "Bxs", "Bys", "Axs", "Ays", "maxFC", "minK", "orthogonal", "rx", "ry", "useGPU", "n"), envir=environment())
    gpuR::setContext(3L)
...
selectBestUDGPCorMatrices <- function(phen.p, alpha, maxFC, fileID, resultsDir,
                                      savePlots=TRUE, variables=NULL, debug=TRUE) {
  ...
  for (xyind in 1:dim(xyinds)[2]) {
    ...
    if (file.exists(resultFile)) {
      ...
      selectedInds <- selectBestResult(curResult$results_g, curResult$results_e, curResult$rho_t,
                                       curResult$p.value_t, alpha, maxFC=maxFC, debug=debug)
...
saveEdgeResultsPlot <- function(curResult, folderToSave, fileID, Xind, Yind, Sinds, alpha, maxFC, debug=TRUE) {
    ...
    png(filename=paste0(folderToSave, fileID, "_edge_",Xind,"-",Yind, "_", paste(Sinds, collapse=""), ".png"), width=1280, height=768)
...
selectBestResult <- function(resultsList_g, resultsList_e, rho_t, pvalue_t, alpha, maxFC=0.01, debug=TRUE) {
...
getKinshipList <- function(pedigrees, sampled=NULL) {
...
#' @import foreach
preprocessFamilyBasedUDGs <- function(phen.df, covs.df, pedigrees, sampled, fileID, dirToSave, max_cores=NULL, minK=10, maxFC = 0.01, orthogonal=TRUE, useGPU=FALSE, debug=TRUE) {
  pseq <- 1:ncol(phen.df)
  N <- dim(phen.df)[1]
  xyinds <- combn(pseq,2)
      ...
      foreach(jobi = batches[[batchId]]) %dopar% {
        ...
        if (file.exists(fileToSave)) {
          ...
          if (ret_g == "error" || ret_e == "error") {
            if (debug) {
              ...
              cat(paste0(Sys.getpid(), " - redoing computation for ", x, ".", y, "|{",paste(S,   collapse=","), "}...\n"))
            }
            ...
            if (debug) {
              cat(paste0(Sys.getpid(), " - updating computation for ", x, ".", y, "|{",paste(S, collapse=","), "}...\n"))
            }
          }
        }
        ...
        if (processFile) {
          ...
          curResult <- getFamilyBasedPCorResults(x, y, S, phen.df, covs.df, sampledPed, Phi2, Z, minK,
                                                 maxFC, orthogonal, fileID, dirToSave, useGPU, debug)
          if (!is.null(fileID) && !is.null(dirToSave)) {
...
preprocessFamilyBasedDAGs <- function(phen.df, covs.df, pedigrees, sampled, alpha,
    ...
    orthogonal=TRUE, savePlots=FALSE, useGPU=FALSE, debug=TRUE) {
      ...
      if (remainingEdges > 0) {
        ...
        if (dim(jobqueue)[1] > 0) {
          ...
          for (batchId in 1:length(batches)) {
            curResults <- c()
            jobi = 0 # a hack to avoid problems when checking --as-cran
            curResults <- foreach(jobi = batches[[batchId]], .combine='rbind') %dopar% {
              ...
              ret <- familyBasedPCorTest(x, y, S, phen.df, covs.df, sampledPed, Phi2, Z,
                ...
                dirToSave, fileID, suffStat$savePlots, useGPU=useGPU, debug=debug)
...
familyBasedPCorTest <- function(x, y, S, phen.df, covs.df, pedigrees, Phi2, Z,
        ...
        dirToSave=NULL, fileID=NULL, savePlots=FALSE, useGPU=FALSE, debug=TRUE) {
  ...
  curResult <- getFamilyBasedPCorResults(x, y, S, phen.df, covs.df, pedigrees, Phi2, Z, minK, maxFC,
                                         orthogonal, fileID, dirToSave, useGPU, debug)
  results <- selectFamilyBasedPCor(curResult, alpha, maxFC, curResultFileName=fileToSave, debug=debug)

  if (savePlots && !is.null(fileID) && !is.null(dirToSave)) {
      if (debug) {
        cat("Generating result plot...")
      }
      saveEdgeResultsPlot(curResult, dirToSave, fileID, x, y, S, alpha, maxFC, debug=debug)
  }
...
selectFamilyBasedPCor <- function(curResult, alpha, maxFC, curResultFileName="", debug=TRUE) {
  ...
  selectedInds <- selectBestResult(curResult$results_g, curResult$results_e,
    curResult$rho_t, curResult$p.value_t, alpha, maxFC, debug=debug)
...
getPCorInfo <- function(csvfile, x, y, S, dagg, dage, dagt, debug=TRUE) {
  retvalue = FALSE;
  ...
  if (file.exists(csvfile)) {
    ...
    if (length(ind) > 1) {
      ...
      write.table(preprocessedPvalues, file = csvfile, sep = ";", row.names=FALSE, col.names = TRUE, append=FALSE, quote=FALSE)
      ind = ind[1]
    }
...
generateFamilyDAG <- function(phen.df, covs.df, pedigrees, sampled=NULL,
    preprocessedPvaluesCSVFile, type, alpha, maj.rule=FALSE,
    minK=10, maxFC = 0.01, orthogonal=TRUE,
    dirToSave, fileID, savePlots, useGPU=FALSE, debug=TRUE) {
  labels <- c("x", "y", "S","pcort", "pcort_p.value",
        "pcorg", "pcorg_p.value",
        "pcore", "pcore_p.value",
        "se", "sg")
  ...
  suffStat <- list(
    ...
    if (suffStat$maj.rule == FALSE) {
      ...
      dag <- pc(suffStat=suffStat, indepTest = familyBasedCITest,
        alpha=alpha, labels = paste0(colnames(phen.df), "_", type), verbose = TRUE,
        skel.method="stable", solve.confl=TRUE, maj.rule = TRUE)
    }
